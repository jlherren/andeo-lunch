...
describe('Absences', () => {
    ...
    it('List absences when empty', async () => {
        let response = await agent.get(`/api/users/${user1.id}/absences`);
        expect(response.status).to.equal(200);
        expect(response.body.absences).to.have.lengthOf(0);
    });

    it('Saves single day absence', async () => {
        let response = await agent.post(`/api/users/${user1.id}/absences`).send({
            start: '2020-01-04',
            end:   '2020-01-04',
        });
        expect(response.status).to.equal(201);
        response = await agent.get(`/api/users/${user1.id}/absences`);
    ...
    it('Deletes an absence', async () => {
        ...
        response = await agent.delete(location);
    ...
    it('Does not return absence for wrong user', async () => {
        ...
        expect(response.status).to.equal(201);
        response = await agent.get(`/api/users/${user2.id}/absences`);
        expect(response.body.absences).to.have.lengthOf(0);
    });
